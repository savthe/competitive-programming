...
fn solve() {
    let (n, k) = inp::read2::<us, us>();
    let mut a = inp::read_vec::<us>();
    a.sort_unstable();

    // В отсортированном массиве сложностей нам подойдет самая длинная последовательность задач,
    // в которой две последовательные задачи отличаются не более, чем на k. max - длина этой
    // последовательности.
    let mut max = 0;

    // Длина текущей сбалансированной подпоследовательности.
    let mut counter = 1;
    for (&prev, &cur) in a.iter().zip(a.iter().skip(1)) {
       if cur - prev > k {
           max = max.max(counter);
           counter = 0;
       }
